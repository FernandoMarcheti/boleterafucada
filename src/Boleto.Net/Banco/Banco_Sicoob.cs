...
namespace BoletoNet
{
    ...
    internal class Banco_Sicoob : AbstractBanco, IBanco
    {
        ...
        public string GerarDetalheRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, string obs)
        {
            try
            {
                string _detalhe = " ";
                ...
                switch (tipoArquivo)
                {
                    ...
                    case TipoArquivo.CNAB240:
                        ...
                        break;
                    case TipoArquivo.CNAB400:
                        _detalhe = GerarDetalheRemessaCNAB400(boleto, numeroRegistro, tipoArquivo, obs);
                        break;
                    case TipoArquivo.Outro:
                        throw new Exception("Tipo de arquivo inexistente.");
                }
        ...
        private string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, string obs="")
        {
            //Variaveis
            ...
            try
            {
                ...
                if (boleto.Instrucoes.Count > 0)
                    _detalhe.Append(Utils.FitStringLength(boleto.Instrucoes[0].Codigo.ToString(), 2, 2, '0', 0, true, true, true));
                else
                    _detalhe.Append("07"); //Não há instruções
                ...
                _detalhe.Append(Utils.FitStringLength(obs, 40, 40, ' ', 0, true, true, false)); //Posição 352 a 391 - OBSERVACOES
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio, int contadorLoteServico)
        {
            try
            {
                string detalhe = Utils.FormatCode(Codigo.ToString(), 3); //Posição 001 a 003   Código do Sicoob na Compensação: "756"
                detalhe += Utils.FormatCode(contadorLoteServico.ToString(), "0", 4, true); //Posição 004 a 007   Número Sequencial
                ...
                detalhe += new string('0', 5);//Posição 101 a 105     Agência Encarregada da Cobrança: "00000"
                ...
                detalhe += "N";  //Posição 109 Identificação do título Aceito/Não Aceito  TODO:Deivid
                ...
                var dataJuros = boleto.DataVencimento.AddDays(1);
                detalhe += Utils.FormatCode(dataJuros.ToString("ddMMyyyy"), 8);  //Posição 119 a 126  - Data do Juros de Mora: preencher com a Data de Vencimento do Título
                detalhe += Utils.FormatCode(boleto.JurosMora.ToString().Replace(",", "").Replace(".", ""), 15);   //Posição 127 a 141  - Data do Juros de Mora: preencher com a Data de Vencimento do Título
                detalhe += "0"; //Posição 142  - Código do desconto

                detalhe += new string('0', 8); //Posição 143 a 150  - Data do Desconto 1
        ...
        public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, int contadorLoteServico)
        {
            try
            {
        ...
        public override string GerarDetalheSegmentoRRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, int contadorLoteServico)
        {
            try
            {
                string detalhe = Utils.FormatCode(Codigo.ToString(), 3); //Posição 001 a 003   Código do Sicoob na Compensação: "756"
                ...
                detalhe += " ";  //Posição 015 Uso Exclusivo FEBRABAN/CNAB: Brancos
                detalhe += "01"; //Posição 016 a 017       '01'  =  Entrada de Títulos
                detalhe += "0"; //Posição 18  - Código do desconto
                detalhe += new string('0', 8); //Posição 19 a 26  - Data do Desconto 2
                ...
                detalhe += "0"; //Posição 118  - Código da desconto
                ...
                var dataMulta = boleto.DataVencimento.AddDays(1);
                detalhe += Utils.FormatCode(dataMulta.ToString("ddMMyyyy"), 8);  //Posição 119 a 126  - Data do Juros de Mora: preencher com a Data de Vencimento do Título
