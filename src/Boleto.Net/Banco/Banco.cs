...
namespace BoletoNet
{
	public class Banco : AbstractBanco, IBanco
	{
        ...
        public string GerarDetalheRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, string obs)
        {
            try
            {
                return _IBanco.GerarDetalheRemessa(boleto, numeroRegistro, tipoArquivo, obs);
            }
            catch (Exception ex)
            {
		...
		public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio, int contadorLoteServico)
		{
			try
			{
				return _IBanco.GerarDetalheSegmentoPRemessa(boleto, numeroRegistro, numeroConvenio, contadorLoteServico);
			}
			catch (Exception ex)
			{
				throw new Exception("Erro durante a geração dos registros de DETALHE do arquivo de REMESSA.", ex);
		...
		public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, int contadorLoteServico)
		{
			try
			{
				return _IBanco.GerarDetalheSegmentoQRemessa(boleto, numeroRegistro, tipoArquivo, contadorLoteServico);
			}
			catch (Exception ex)
			{
				throw new Exception("Erro durante a geração dos registros de DETALHE do arquivo de REMESSA.", ex);
		...
		public override string GerarDetalheSegmentoRRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo, int contadorLoteServico)
		{
			try
			{
				return _IBanco.GerarDetalheSegmentoRRemessa(boleto, numeroRegistro, tipoArquivo, contadorLoteServico);
			}
			catch (Exception ex)
			{
				throw new Exception("Erro durante a geração dos registros de DETALHE do arquivo de REMESSA.", ex);
